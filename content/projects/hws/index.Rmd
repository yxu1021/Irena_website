...
Based on the TfL data on how many bikes were hired every single day. We can get the latest data by running the following
...
bike_graph1 <- bike %>% 
  ...
  ungroup %>% 
  group_by(month, year) %>% 
  summarise(expected_rentals = median(expected_rentals), actual_rentals = median(bikes_hired)) %>% 
  mutate(excess_rentals = actual_rentals - expected_rentals)
  
ggplot(bike_graph1, aes(x=month, group=1))+
  geom_ribbon(aes(ymin = ifelse(actual_rentals < expected_rentals, actual_rentals, expected_rentals), ymax = expected_rentals),fill= "green", alpha=0.1)+
  geom_ribbon(aes(ymin=expected_rentals, ymax=ifelse(actual_rentals > expected_rentals, actual_rentals, expected_rentals)),fill="red", alpha=0.1)+
    geom_line(aes(y=expected_rentals), color= "blue", size=0.5)+
...
ggplot(bike_graph2, aes(x=week, group=1))+
